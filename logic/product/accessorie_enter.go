...
func (l *ProductAccessorieEnterLogic) AddProduct(req *types.ProductAccessorieEnterAddProductReq) error {
	...
	for _, p := range enter.Products {
		names[strings.TrimSpace(strings.TrimSpace(strings.ToUpper(p.Name)))] = p
	}
	// 添加配件的结果
	...
	if err := model.DB.Transaction(func(tx *gorm.DB) error {
		for _, p := range req.Products {
			// 检查调拨单是否已经存在该配件
			paap, ok := names[strings.TrimSpace(strings.TrimSpace(strings.ToUpper(p.Name)))]
			if ok {
				// 配件已存在，更新配件
				...
				data := model.ProductAccessorieEnterProduct{
					EnterId:    enter.Id,
					Name:       strings.TrimSpace(strings.TrimSpace(strings.ToUpper(p.Name))),
					Type:       p.Type,
