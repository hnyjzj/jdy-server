...
type OrderSales struct {
	...
	OrderDeposits []OrderDeposit  `json:"order_deposits" gorm:"many2many:order_sales_deposits;"`          // 定金单
	PriceDeposit  decimal.Decimal `json:"price_deposit" gorm:"type:decimal(15,4);not NULL;comment:定金抵扣;"` // 定金抵扣
...
func (OrderSalesProduct) WhereCondition(db *gorm.DB, req *types.OrderSalesDetailWhere) *gorm.DB {
	...
	if req.SalesmanId != "" {
		db = db.Where("order_id IN (SELECT order_id FROM order_sales_clerks WHERE salesman_id = ?)", req.SalesmanId)
	}
	if req.Status != 0 {
		db = db.Where("status = ?", req.Status)
	}
