...
func (p *Product) UpdateStatus(tx *gorm.DB, status enums.ProductStatus, action enums.ProductStatusAction, soure_id string, staff *types.Staff) error {
	if err := p.Status.CanTransitionTo(status); err != nil {
		return err
	}
...
type ProductHistory struct {
	...
	Status    enums.ProductStatus       `json:"status" gorm:"type:tinyint(2);comment:状态;"`      // 状态
	OldStatus enums.ProductStatus       `json:"old_status" gorm:"type:tinyint(2);comment:旧状态;"` // 旧状态
	Action    enums.ProductStatusAction `json:"action" gorm:"type:tinyint(2);comment:操作;"`      // 操作
